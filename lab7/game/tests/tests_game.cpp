...
TEST(SynchronizedArenaTest, NotifyAllThreads)
{
    SynchronizedArena synchronizedArena(500, 500);
    bool notified = false;

    std::thread notifier([&]
                         {
        synchronizedArena.notifyAllThreads();
        notified = true; });
...
TEST(SynchronizedArenaTest, GameOverFlag)
{
    SynchronizedArena synchronizedArena(500, 500);
    EXPECT_FALSE(synchronizedArena.isGameOver());

    synchronizedArena.setGameOver(true);
    EXPECT_TRUE(synchronizedArena.isGameOver());
}
